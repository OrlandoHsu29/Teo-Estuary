...
jieba.load_userdict('./app/teochew_g2p/dict_data/word_dict/jieba_cut.txt')

class pyPengIm():
    def __init__(self, history=False) -> None:
        self._dict_paths = {
            "vocab": "./app/teochew_g2p/dict_data/vocab/origin_vocab.txt",
            "vocab_extension": "./app/teochew_g2p/dict_data/vocab/vocab_extension.txt",
            "word_dict": "./app/teochew_g2p/dict_data/word_dict/dict.txt",
            "teochew_word_dict": "./app/teochew_g2p/dict_data/word_dict/teochew_local_dict.txt",
            "translation_dict": "./app/teochew_g2p/dict_data/word_dict/madr_to_tch.txt",
            "surname_dict": "./app/teochew_g2p/dict_data/vocab/Surname.txt",
            "IPA_dict": "./app/teochew_g2p/dict_data/vocab/IPA_lexicon.txt",
            # "phoneme_dict": "./dict_data/vocab/phone.txt",
            "low_fre_dict": "./app/teochew_g2p/dict_data/vocab/low_fre.txt"
        }
        ...
        if history:
            self.word_dict.update(utils.load_dict("./app/teochew_g2p/dict_data/word_dict/history.txt"))
            self.word_dict.update(utils.load_dict("./app/teochew_g2p/dict_data/word_dict/reign_title.txt"))
    ...
    def _load_accent(self,accent_config_path="./app/teochew_g2p/dict_data/accent_convert/accent.yaml"):
        with open(accent_config_path, 'r', encoding='utf-8') as file:
            accent_config = yaml.safe_load(file)
            accent_dict = {}
            for k,v in accent_config.items():
                accent_dict[k] = (
                        utils.load_dict(os.path.join("./app/teochew_g2p/dict_data/accent_convert",v['path'])),
                        v['name']
                    )
